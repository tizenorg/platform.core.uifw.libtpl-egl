...
__tpl_gbm_display_init(tpl_display_t *display)
{
	...
	{
		__tpl_list_init(&gbm_display->comp.cached_buffers);
	}
	else
...
__tpl_gbm_display_fini(tpl_display_t *display)
{
	...
	if (gbm_display != NULL)
	{
		if (gbm_display->comp.bound_client_display)
			__tpl_gbm_display_unbind_client_wayland_display(display, NULL);

		__tpl_list_fini(&gbm_display->comp.cached_buffers, (tpl_free_func_t) tpl_object_unreference);
...
__tpl_gbm_surface_create_buffer_from_gbm_surface(tpl_surface_t *surface, tpl_bool_t *reset_buffers)
{
	...
	buffer = __tpl_gbm_surface_buffer_cache_find(&gbm_display->comp.cached_buffers, key);
	if (buffer != NULL)
	{
		return buffer;
	}
	...
	if (TPL_TRUE != __tpl_gbm_surface_buffer_cache_add(&gbm_display->comp.cached_buffers, buffer))
	{
...
__tpl_gbm_buffer_destroy_notify(struct wl_listener *listener, void *data)
{
	...
	gbm_buffer = wl_container_of(listener, gbm_buffer, comp.destroy_listener);
	display = gbm_buffer->display;
	key = tbm_bo_export(gbm_buffer->bo);
	gbm_display = (tpl_gbm_display_t *)display->backend.data;
	tpl_object_unreference((tpl_object_t *)gbm_buffer->tpl_buffer);
	__tpl_gbm_surface_buffer_cache_remove(&gbm_display->comp.cached_buffers, key);
}
...
__tpl_gbm_surface_create_buffer_from_wl_tbm(tpl_surface_t *surface, tpl_bool_t *reset_buffers)
{
	...
	else
	{
		...
		gbm_buffer->comp.destroy_listener.notify = __tpl_gbm_buffer_destroy_notify;
		wl_resource_add_destroy_listener((struct wl_resource*)surface->native_handle, &gbm_buffer->comp.destroy_listener);
	}
...
unsigned int __tpl_gbm_display_bind_client_wayland_display(tpl_display_t *tpl_display, tpl_handle_t native_dpy)
{
	...
	tpl_gbm_display->comp.bound_client_display = TPL_TRUE;
	return TPL_TRUE;
}
...
unsigned int __tpl_gbm_display_unbind_client_wayland_display(tpl_display_t *tpl_display, tpl_handle_t native_dpy)
{
	...
	tpl_gbm_display->comp.bound_client_display = TPL_FALSE;
	return TPL_TRUE;
}
#endif

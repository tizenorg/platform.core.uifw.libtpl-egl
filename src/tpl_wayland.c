...
__tpl_wayland_display_roundtrip(tpl_display_t *display)
{
	...
	wl_proxy_set_queue((struct wl_proxy *) callback, wayland_display->app.wl_queue);

	while (ret != -1 && !done)
	{
		ret = wl_display_dispatch_queue(wl_dpy, wayland_display->app.wl_queue);
	}
...
__tpl_wayland_display_init(tpl_display_t *display)
{
	...
	if (__tpl_wayland_display_is_wl_display(display->native_handle))
	{
		...
		wayland_display->app.wl_queue = wl_display_create_queue(wl_dpy);
		if (NULL == wayland_display->app.wl_queue)
			goto free_wl_display;

		wayland_display->app.wl_registry = wl_display_get_registry(wl_dpy);
		if (NULL == wayland_display->app.wl_registry)
			goto destroy_queue;

		wl_proxy_set_queue((struct wl_proxy *)wayland_display->app.wl_registry, wayland_display->app.wl_queue);
	}
	else
		goto free_wl_display;
	...
	return TPL_TRUE;
destroy_queue:
	wl_event_queue_destroy(wayland_display->app.wl_queue);
free_wl_display:
	if (wayland_display != NULL)
	{
...
__tpl_wayland_surface_post(tpl_surface_t *surface, tpl_frame_t *frame)
{
	...
	TPL_LOG(3, "\t buffer(%p, %p) key:%zu", frame->buffer, wayland_buffer->app.wl_proxy, frame->buffer->key);
	...
	wl_surface_attach(wl_egl_window->surface,
			(void *)wayland_buffer->app.wl_proxy,
			wl_egl_window->dx,
			wl_egl_window->dy);
...

	{
		...
		wl_proxy_set_queue((struct wl_proxy *)frame_callback, wayland_display->app.wl_queue);
	}
	wl_surface_commit(wl_egl_window->surface);
...
__tpl_wayland_surface_begin_frame(tpl_surface_t *surface)
{
	...
	while (__tpl_wayland_surface_get_idle_buffer_idx(wayland_surface, TPL_BUFFER_ALLOC_SIZE_APP) == -1)
	{
		...
		if (wl_display_dispatch_queue(surface->display->native_handle, wayland_display->app.wl_queue) == -1)
		{
...
/* TODO: If HW support getting of  gem memory size,
	...
	wl_proxy_set_queue(wl_proxy, wayland_display->app.wl_queue);
	...
	wayland_buffer->app.wl_proxy = wl_proxy;
...
__tpl_wayland_buffer_fini(tpl_buffer_t *buffer)
{
	...
	if (buffer->backend.data)
	{
		...
		if (wayland_buffer->app.wl_proxy != NULL)
			wayland_tbm_client_destroy_buffer(wayland_display->wl_tbm_client, (void *)wayland_buffer->app.wl_proxy);

...
static void
__cb_client_tbm_surface_queue_reset_callback(tbm_surface_queue_h tbm_queue,
					     void *data);
...
__tpl_wayland_surface_init(tpl_surface_t *surface)
{
	...
	if (!wayland_tbm_client_add_reset_queue_cb(wayland_display->wl_tbm_client,
					      wayland_surface->tbm_queue,
					      __cb_client_tbm_surface_queue_reset_callback,
					      (void *)wayland_surface))
	{
		TPL_ERR("Failed to register tbm_surface_queue reset callback.");
	}

#if USE_HISTORY_LIST
	wayland_surface->history = __tpl_list_alloc();
#endif
...
#if USE_HISTORY_LIST
	__tpl_list_free(wayland_surface->history, NULL);
        wayland_surface->history = NULL;
#endif
	free(wayland_surface);
				     ...
				     int num_rects, const int *rects)
{
	...
	TPL_LOG(9, "[ENQUEUE] tbm_surface(%p) wl_buffer(%p) bo(%d)",
		tbm_surface, wayland_buffer->wl_proxy,
	...
	TPL_LOG(9, "[ACQUIRE] tbm_surface(%p) wl_buffer(%p) bo(%d)",
			tbm_surface, wayland_buffer->wl_proxy, tbm_bo_export(wayland_buffer->bo));
...
__tpl_wayland_surface_dequeue_buffer(tpl_surface_t *surface)
{
...
#else
		if (wl_display_dispatch((struct wl_display*)
					surface->display->native_handle) == -1) {
			TPL_OBJECT_LOCK(surface);
			return NULL;
		}
#endif
	...
	tbm_surface_internal_ref(tbm_surface);
#if USE_HISTORY_LIST
	__tpl_list_push_back(wayland_surface->history, (void*)tbm_surface);
#endif

	if ((wayland_buffer =
		     __tpl_wayland_get_wayland_buffer_from_tbm_surface(tbm_surface)) != NULL) {
		TPL_LOG(9, "[DEQUEUE] tbm_surface(%p) wl_buffer(%p) bo(%d)",
			tbm_surface, wayland_buffer->wl_proxy,
                        tbm_bo_export(wayland_buffer->bo));
		return tbm_surface;
	}
...
#else
	wl_proxy_set_queue(wl_proxy, NULL);
#endif
...
__tpl_wayland_buffer_free(tpl_wayland_buffer_t *wayland_buffer)
{
	...
	TPL_LOG(9, "wayland_tbm_client_destroy_buffer | wl_buffer(%p)",wayland_buffer->wl_proxy);
...
__cb_client_buffer_release_callback(void *data, struct wl_proxy *proxy)
{
	...
	if (wayland_buffer) {
		TPL_LOG(9, "[RELEASE] tbm_surface(%p) wl_buffer(%p) bo(%d)",
			tbm_surface, wayland_buffer->wl_proxy, tbm_bo_export(wayland_buffer->bo));
					     ...
					     void *data)
{
	...
	tpl_wayland_surface_t *wayland_surface = (tpl_wayland_surface_t *)data;

	if (wayland_surface->history != NULL)
	{
		while (!__tpl_list_is_empty(wayland_surface->history))
		{
			tbm_surface_h tbm_surface =
                                (tbm_surface_h)__tpl_list_pop_front(wayland_surface->history, NULL);
			TPL_LOG(9, "[RESET_CB] tbm_surface(%p) bo(%d)", tbm_surface,

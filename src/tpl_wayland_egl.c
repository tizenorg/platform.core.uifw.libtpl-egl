...
static void __tpl_wayland_egl_buffer_free(tpl_wayland_egl_buffer_t *wayland_egl_buffer);
...
__tpl_wayland_egl_surface_init(tpl_surface_t *surface)
{
	...
	if (wl_egl_window->surface)
		wayland_egl_surface->tbm_queue = wayland_tbm_client_create_surface_queue(wayland_egl_display->wl_tbm_client,
				     ...
				     int num_rects, const int *rects)
{
	...
	tsq_err = tbm_surface_queue_enqueue(wayland_egl_surface->tbm_queue, tbm_surface);
	...
	/* deprecated */
	tsq_err = tbm_surface_queue_acquire(wayland_egl_surface->tbm_queue, &tbm_surface);
	if (tsq_err != TBM_SURFACE_QUEUE_ERROR_NONE) {
...
__tpl_wayland_egl_surface_dequeue_buffer(tpl_surface_t *surface)
{
	...
	tsq_err = tbm_surface_queue_dequeue(wayland_egl_surface->tbm_queue, &tbm_surface);
	if (!tbm_surface) {
	...
	__tpl_wayland_egl_set_wayland_buffer_to_tbm_surface(tbm_surface, wayland_egl_buffer);
...
__tpl_wayland_egl_buffer_free(tpl_wayland_egl_buffer_t *wayland_egl_buffer)
{
	...
	wl_display_flush((struct wl_display *)wayland_egl_buffer->display->native_handle);
...
__tpl_display_init_backend_wayland_egl(tpl_display_backend_t *backend)
{
	...
	backend->type = TPL_BACKEND_WAYLAND_EGL;
	backend->data = NULL;

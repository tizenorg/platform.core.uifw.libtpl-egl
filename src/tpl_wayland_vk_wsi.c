...
#if 0
	/* consider saving of current_buffer_list for resizing window */
...
static void __tpl_wayland_vk_wsi_buffer_free(tpl_wayland_vk_wsi_buffer_t *wayland_vk_wsi_buffer);
static tpl_result_t __tpl_wayland_vk_wsi_surface_destroy_swapchain(tpl_surface_t *surface);
...
__tpl_wayland_vk_wsi_get_wayland_buffer_from_tbm_surface(tbm_surface_h surface)
{
	...
	tbm_surface_internal_get_user_data(surface, KEY_tpl_wayland_vk_wsi_buffer, (void **)&buf);
	return buf;
}
...
static tpl_result_t
__tpl_wayland_vk_wsi_display_filter_config(tpl_display_t *display, int *visual_id,
				    int alpha_size)
{
	TPL_IGNORE(display);
...
#if 0
	wayland_vk_wsi_surface->resized = TPL_FALSE;
	/* consider saving of current_buffer_list for resizing window */
	wayland_vk_wsi_surface->current_buffer = NULL;
#endif
				     ...
				     int num_rects, const int *rects)
{
	...
	tsq_err = tbm_surface_queue_enqueue(wayland_vk_wsi_surface->tbm_queue, tbm_surface);
...
__tpl_wayland_vk_wsi_surface_validate(tpl_surface_t *surface)
{
	...
	tpl_wayland_vk_wsi_surface_t *wayland_vk_wsi_surface =
		(tpl_wayland_vk_wsi_surface_t *)surface->backend.data;
	if (wayland_vk_wsi_surface->resized) return TPL_FALSE;
#endif
...
__tpl_wayland_vk_wsi_surface_dequeue_buffer(tpl_surface_t *surface)
{
	...
	if (wayland_vk_wsi_surface->resized == TPL_TRUE) {
		struct wl_egl_window *wl_egl_window = (struct wl_egl_window *)
						      surface->native_handle;
		int width, height, format;
		width = wl_egl_window->width;
		height = wl_egl_window->height;
		format = tbm_surface_queue_get_format(wayland_vk_wsi_surface->tbm_queue);

		/* TODO: In dequeue_buffer case, does it explictly need on unref of current_buffer? */
	    /* consider saving of current_buffer_list for resizing window */
		if (wayland_vk_wsi_surface->current_buffer)
			tbm_surface_internal_unref(wayland_vk_wsi_surface->current_buffer);

		tbm_surface_queue_reset(wayland_vk_wsi_surface->tbm_queue, width, height, format);
		wayland_vk_wsi_surface->resized = TPL_FALSE;
		surface->width = width;
	...
	tsq_err = tbm_surface_queue_dequeue(wayland_vk_wsi_surface->tbm_queue, &tbm_surface);
	if (!tbm_surface) {
	...
	if ((wayland_vk_wsi_buffer =
		     __tpl_wayland_vk_wsi_get_wayland_buffer_from_tbm_surface(tbm_surface)) != NULL) {
		return tbm_surface;
	}
...
#if 0
	wayland_vk_wsi_surface->current_buffer = tbm_surface;
#endif

	__tpl_wayland_vk_wsi_set_wayland_buffer_to_tbm_surface(tbm_surface, wayland_vk_wsi_buffer);
						   ...
						   int *buffer_count)
{
	...
	wayland_vk_wsi_surface = (tpl_wayland_vk_wsi_surface_t *)surface->backend.data;
	swapchain_buffers = (tbm_surface_h*)calloc(wayland_vk_wsi_surface->buffer_count, sizeof(tbm_surface_h));
...
static void
__tpl_wayland_vk_wsi_buffer_free(tpl_wayland_vk_wsi_buffer_t *wayland_vk_wsi_buffer)
{
	TPL_ASSERT(wayland_vk_wsi_buffer);
	...
	wl_display_flush((struct wl_display *)wayland_vk_wsi_buffer->display->native_handle);
...
__tpl_surface_init_backend_wayland_vk_wsi(tpl_surface_backend_t *backend)
{
	...
	backend->get_swapchain_buffers = __tpl_wayland_vk_wsi_surface_get_swapchain_buffers;

...
bool
tpl_surface_create_test(TPLNativeWnd* wnd)
{
	TPL_CHK_PARAM(!wnd);
...
bool
tpl_surface_validate_test(TPLNativeWnd* wnd)
{
	TPL_CHK_PARAM(!wnd);
	LOG("INFO", LOG_LEVEL_LOW , "-------begin:%s-------",__func__);
	bool ret = true;
...
bool
tpl_surface_get_arg_test(TPLNativeWnd* wnd)
{
	TPL_CHK_PARAM(!wnd);
	LOG("INFO", LOG_LEVEL_LOW , "-------begin:%s-------",__func__);
	bool ret = true;
	...
	//if(test_result != TPL_ERROR_NONE || (width != tpl_surf->width ||\
	//height != tpl_surf->height))
...
bool
tpl_surface_dequeue_and_enqueue_buffer_test(TPLNativeWnd* wnd)
{
	TPL_CHK_PARAM(!wnd);
	LOG("INFO", LOG_LEVEL_LOW , "-------begin:%s-------",__func__);
	bool ret = true;

...
void TPLTest::TearDown() {
    finalize();
}

void TPLTest::initialize() {
    ...
    tpl_display = tpl_display_create(TPL_BACKEND_WAYLAND, (tpl_handle_t) wl_disp.display);
    tpl_surface = tpl_surface_create(tpl_display, (tpl_handle_t)wl_win.native, TPL_SURFACE_TYPE_WINDOW, TBM_FORMAT_ARGB8888);
}

void TPLTest::finalize() {
    if (tpl_surface != NULL) {

...
bool tpl_object_get_type_test(TPLNativeWnd* wnd )
{
    ...
    wnd->tpl_surf = NULL;
    wnd->tpl_surf = tpl_surface_create(wnd->tpl_display, (tpl_handle_t)wnd->wnd, TPL_SURFACE_TYPE_WINDOW, TBM_FORMAT_ARGB8888);
...
bool tpl_object_userdata_test(TPLNativeWnd* wnd )
{
    ...
    //2. set user data
    tpl_object_set_user_data((tpl_object_t *)wnd->tpl_display, &key, (void *)wnd->dpy,NULL);

    //3.get user data
    void* get_dpy = NULL;
    get_dpy = (void *)tpl_object_get_user_data((tpl_object_t *)wnd->tpl_display, &key);
    if(get_dpy == NULL)
    {
